...
function filtro_pokemon($atts)
{
    ...
    foreach ($pokemon_ordenado as $num => $valor) {
        $output .= '<tr>
        <td><img class="tabla-imagen-pokemon" alt="Imagen de ' . $valor['nombre'] . '" src="' .
            $valor['sprite'] . '" /></td>
        <td>' . $num . '</td>
        <td nombre="' . $valor['nombre'] . '">' . $valor['nombre'] . '</td>
        <td>';
        foreach ($valor['tipos'] as $tipo) {
            $output .= '<img class="tabla-imagen-tipo" src="' . get_bloginfo('wpurl') . "/wp-content/plugins/aopokedex/pokemon_types/" . $tipo . '.svg" />';
        }
        $output .= '</td></tr>';
    }
